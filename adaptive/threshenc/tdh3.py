...
def hashH(x, L, u, w, u1, w1): #H_2
    #assert len(x) == 32
    #return group.hash(x.encode('utf-8')+L+serialize(u)+serialize(w)+serialize(u1)+serialize(w1))
...
def xor(x,y):
    ...
    if isinstance(x,str) and isinstance(y,str):
        return ''.join(chr(ord(x_)^ord(y_)) for x_,y_ in zip(x,y))
    print ("length of str: %d"%len(''.join(chr(x_^y_) for x_,y_ in zip(x,y))))
    print ("length of byte: %d"%len((''.join(chr(x_^y_) for x_,y_ in zip(x,y))).encode('utf-8')))
    return (''.join(chr(x_^y_) for x_,y_ in zip(x,y))).encode('utf-8')'''
